...
INPUT_FILE_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "new_mapping_2.csv"
)  # "test_data","2p_more_than_2_tgt_p.csv")


def remove_done_wells_from_csv(
    prev_csv_absolute_path: str, done_map_file_path: str, output_csv_path: str
) -> dict:
    """
...
def get_last_tgt_well_used_in_current_plate(
    map_file_path: str, new_input_csv_file_path: str
):
    plate = None
    last_well_id = None
    ...
    for i in range(len(filtered_unique_well_poss)):
        ...
        map_keys_of_interest = [
            k for k in done_map.keys() if k.startswith(filtered_plate)
        ]
