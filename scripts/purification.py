...
def run(deck: dict, state: dict, state_file_path: str, run_dir_path: str):
    ...
    logger.debug("Getting number of tubes from prompt...")

    pools = hp.prompt_int("Pools to purify", 8)

    eppies = [f"P{i}" for i in range(1, pools + 1)]

    logger.debug(f"Pools to purify: {eppies}")

    # Assign labware to deck positions

    logger.info("Assigning labware...")

    source_eppies = dk.get_labware_list(deck, ["C1"], EppiCarrier24)[0]

    dest_dw_plate = dk.get_labware_list(
        deck,
        ["E2"],
        Plate384,
        [8],
